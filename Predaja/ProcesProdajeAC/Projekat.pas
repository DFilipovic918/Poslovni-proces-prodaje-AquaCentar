...
type
  TPendingReservationType = (prtNone, prtActivity, prtEquipment);
...
type
  ...
  TfMain = class(TForm)
    ...
    Image29: TImage;
    Label3: TLabel;
    finishReservationButton: TCornerButton;
    tiSuccessPayment: TTabItem;
    Image39: TImage;
    Image45: TImage;
    Label6: TLabel;
    CornerButton1: TCornerButton;
    incorrectInfo: TLabel;
    returnDate: TLabel;
    viewReservedUsersButton: TCornerButton;
    tiReservedUsers: TTabItem;
    Image46: TImage;
    reservedUsersBox: TScrollBox;
    backToSelectedTermin: TImage;
    ...
    procedure AddReservedEquipmentRectangle(const EquipID, EquipName: string; Qty: Integer; ReservedDate: TDateTime);
    ...
    procedure cardNumberChangeTracking(Sender: TObject);
    procedure cardCodeChangeTracking(Sender: TObject);
    procedure cardDateChangeTracking(Sender: TObject);
    procedure CornerButton1Click(Sender: TObject);
    procedure Image26Click(Sender: TObject);
    procedure paymentBackClick(Sender: TObject);
    procedure finishReservationButtonClick(Sender: TObject);
    procedure cardInputButtonClick(Sender: TObject);
    procedure ClearPaymentEdits;
    procedure backToSelectedTerminClick(Sender: TObject);
    procedure viewReservedUsersButtonClick(Sender: TObject);
    procedure LoadReservedUsersForSelectedActivity;
    procedure AddReservedUserRectangle(const FullName: string; TicketCount: Integer);
  private
  SelectedDate: TDateTime;
    ...
    PendingReservationType: TPendingReservationType;
    PaymentBackTab: TTabItem;
    PendingEquipmentQty: Integer;
...
begin
  ...
  cardNumber.KeyboardType := TVirtualKeyboardType.NumberPad;
  cardNumber.MaxLength := 19;
  cardNumber.OnChangeTracking := cardNumberChangeTracking;

  cardCode.KeyboardType := TVirtualKeyboardType.NumberPad;
  cardCode.MaxLength := 3;
  cardCode.OnChangeTracking := cardCodeChangeTracking;

  cardDate.KeyboardType := TVirtualKeyboardType.NumberPad;
  cardDate.MaxLength := 5;
  cardDate.OnChangeTracking := cardDateChangeTracking;

  postalCode.KeyboardType := TVirtualKeyboardType.NumberPad;
  numberForCard.KeyboardType := TVirtualKeyboardType.PhonePad;

  PendingReservationType := prtNone;
  PaymentBackTab := nil;
  PendingEquipmentQty := 0;
...
procedure TfMain.Image26Click(Sender: TObject);
begin
tcMain.ActiveTab := tiSuccess;
end;

procedure TfMain.Image27Click(Sender: TObject);
begin
tcMain.ActiveTab := tiPayment;
end;
...
procedure TfMain.backToSelectedTerminClick(Sender: TObject);
begin
...
begin
  ...
  PendingEquipmentQty := Qty;
  PendingReservationType := prtEquipment;
  PaymentBackTab := tiEquipmentReservation;

  tcMain.ActiveTab := tiPaymentChoice;
end;
...
begin
  if Assigned(userCalendar) then
    userCalendar.Date := Date;

  if Assigned(adminCalendar) then
    adminCalendar.Date := Date;
...
begin
  ...
  begin
    ...
    if Qty <= 0 then
    begin
      ShowMessage('Broj karata mora biti veći od 0.');
      Exit;
    end;

    if Qty > E.Tickets then
    begin
  ...
  PendingReservationType := prtActivity;
  PaymentBackTab := tiCheck;
...
begin
  ...
  begin
    ...
    SQL.Clear;
    SQL.Text :=
      'SELECT O.Id_Opreme, O.Naziv, R.Kolicina, R.Datum_vreme ' +
    ...
    begin
      ...
      ReservedDate := FieldByName('Datum_vreme').AsDateTime;
      AddReservedEquipmentRectangle(EquipID, EquipName, Qty, ReservedDate);
      Next;
    end;
...
procedure TfMain.AddReservedEquipmentRectangle(const EquipID, EquipName: string; Qty: Integer; ReservedDate: TDateTime);
var
  R: TRectangle;
  L: TLabel;
begin
...
var
  EquipID: string;
  ReservedDate, ReturnD: TDateTime;
begin
  EquipID := TControl(Sender).TagString;
  SelectedEquipmentID := EquipID;

  OpenEquipmentDetails(EquipID);

  ReservedDate := TControl(Sender).Tag;
  ReturnD := IncDay(ReservedDate, 7);

  returnDate.Text := 'Datum vraćanja: ' + FormatDateTime('dd.mm.yyyy', ReturnD);
...
begin
  Result := (S <> '') and (OnlyDigits(S) = S);
end;

procedure TfMain.cardNumberChangeTracking(Sender: TObject);
var
  Digits, Formatted: string;
  I: Integer;
begin
  Digits := OnlyDigits(cardNumber.Text);
  ...
  begin
    ...
    if (I mod 4 = 0) and (I < Length(Digits)) then
      Formatted := Formatted + ' ';
  end;

  cardNumber.Text := Formatted;
  cardNumber.CaretPosition := Length(cardNumber.Text);
end;

procedure TfMain.cardCodeChangeTracking(Sender: TObject);
var
  Digits: string;
begin
  Digits := OnlyDigits(cardCode.Text);

  if Length(Digits) > 3 then
    Digits := Copy(Digits, 1, 3);

  cardCode.Text := Digits;
  cardCode.CaretPosition := Length(cardCode.Text);
end;

procedure TfMain.cardDateChangeTracking(Sender: TObject);
var
  Digits: string;
begin
  Digits := OnlyDigits(cardDate.Text);

  if Length(Digits) > 4 then
    Digits := Copy(Digits, 1, 4);

  if Length(Digits) >= 3 then
    cardDate.Text := Copy(Digits, 1, 2) + '/' + Copy(Digits, 3, 2)
  else
    cardDate.Text := Digits;

  cardDate.CaretPosition := Length(cardDate.Text);
end;

procedure TfMain.cardInputButtonClick(Sender: TObject);
var
  CardDigits: string;
  Month, Year: Integer;
  CardValid: Boolean;
begin
  ...
  CardDigits := OnlyDigits(cardNumber.Text);

  CardValid :=
    (Trim(cardName.Text) <> '') and
    (Length(CardDigits) = 16) and
    (Length(cardDate.Text) = 5) and
    (cardDate.Text[3] = '/') and
    (Length(cardCode.Text) = 3) and
    IsOnlyDigits(cardCode.Text) and
    (Trim(postalCode.Text) <> '') and
    IsOnlyDigits(postalCode.Text) and
    (Trim(numberForCard.Text) <> '') and
    IsOnlyDigits(numberForCard.Text);

  if CardValid then
  begin
    Month := StrToIntDef(Copy(cardDate.Text, 1, 2), 0);
    Year := StrToIntDef(Copy(cardDate.Text, 4, 2), -1);
  ...
  incorrectInfo.Visible := False;
  tcMain.ActiveTab := tiSuccessPayment;
end;

procedure TfMain.paymentBackClick(Sender: TObject);
begin
  if Assigned(PaymentBackTab) then
    tcMain.ActiveTab := PaymentBackTab
  else
    tcMain.ActiveTab := tiTickets;
end;

procedure TfMain.finishReservationButtonClick(Sender: TObject);
begin
  case PendingReservationType of
    prtActivity:
      begin
    ...
    prtEquipment:
      begin
        ReserveEquipmentF(SelectedEquipmentID, PendingEquipmentQty);
        reserveQtyEdit.Text := '';
      end;
  else
    ShowMessage('Nema rezervacije za završavanje.');
    Exit;
  end;
...
procedure TfMain.ClearPaymentEdits;
begin
  cardName.Text := '';
  cardNumber.Text := '';
  cardDate.Text := '';
...
procedure TfMain.viewReservedUsersButtonClick(Sender: TObject);
begin
  if SelectedActivityID = '' then
  begin
    ShowMessage('Nije izabrana aktivnost.');
    Exit;
  end;

  LoadReservedUsersForSelectedActivity;
  tcMain.ActiveTab := tiReservedUsers;
end;

procedure TfMain.LoadReservedUsersForSelectedActivity;
begin
  if Assigned(reservedUsersBox) and Assigned(reservedUsersBox.Content) then
    ClearLayoutSafe(reservedUsersBox.Content);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Text :=
      'SELECT K.Ime, K.Prezime, COUNT(*) AS BrojKarata ' +
      'FROM RezervacijaAktivnosti R ' +
      'JOIN Korisnik K ON K.ID_kor = R.ID_kor ' +
      'WHERE R.ID_aktivnost = :aktivnost ' +
      'GROUP BY K.Ime, K.Prezime ' +
      'ORDER BY K.Ime, K.Prezime';

    ParamByName('aktivnost').AsString := SelectedActivityID;
    Open;

    while not Eof do
    begin
      AddReservedUserRectangle(
        FieldByName('Ime').AsString + ' ' + FieldByName('Prezime').AsString,
        FieldByName('BrojKarata').AsInteger
      );
...
procedure TfMain.AddReservedUserRectangle(const FullName: string; TicketCount: Integer);
var
  R: TRectangle;
  L: TLabel;
begin
  R := TRectangle.Create(reservedUsersBox);
  R.Parent := reservedUsersBox;
  R.Align := TAlignLayout.Top;
  R.Height := 70;
  R.Margins.Top := 4;
  R.Margins.Bottom := 4;
  ...
  L.Text :=
    FullName + sLineBreak +
    'Broj karata: ' + IntToStr(TicketCount);
end;
